...
# expession (`$1`, `$2`, etc.).
- template: '^(\w+)\.(leaf1)\.eu\.example\.com:([0-9]+)$'

  # Optional web proxy address to use for proxy jump (`--jumphost`).
  #
  ...
  # root proxy. The user's root cluster session is used to retrieve leaf cluster 
  # certificates before connecting to the jump host.
  proxy: "leaf1.eu.example.com:443"

  # Optional cluster name to connect to (`--cluster`).
  #
  # Cluster can be used to connect to leaf nodes from the root proxy without
  # first logging in to the leaf cluster. The user's root cluster session is
  # used to retrieve leaf cluster certificates before connecting to the leaf
  # node. This may be useful in cases where proxy jump is not usable, such as
  # when the leaf clusters do not have public proxies.
  cluster: "$2"
...
```bash
$ tsh ssh -J {{proxy}} node1.leaf1.eu.example.com
# becomes
...
Additionally, it will not replace any values which were explicitly set by the client.

```bash
$ tsh ssh -J root.us.example.com:443 node1.leaf1.eu.example.com
# becomes
$ tsh ssh -J root.us.example.com:443 --cluster leaf1 node1
```
...
```bash
$ tsh ssh node1.leaf1.eu.example.com
# is equivalent to
$ ssh node1.leaf1.eu.example.com
```
